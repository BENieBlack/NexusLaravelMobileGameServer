 ...
 *
 * シャーディング対応: trx1/trx2 両方に作成する
 */
return new class extends Migration
{
    protected array $connections = ['trx1', 'trx2'];
    ...
    public function up(): void
    {
        foreach ($this->connections as $connection) {
            ...
            // trx_reward_track（プレイヤーの進捗値）
            // ------------------------------------------------
            Schema::connection($connection)->create('trx_reward_track', function (Blueprint $table) {
            ...
            // ラインを購入すると1レコード追加される
            // ------------------------------------------------
            Schema::connection($connection)->create('trx_reward_track_line', function (Blueprint $table) {
                $table->id()->comment('自動採番ID');
                $table->unsignedBigInteger('sys_player_id')->comment('sys_playerテーブルのID');
            ...
            // ------------------------------------------------
            Schema::connection($connection)->create('trx_reward_track_milestone', function (Blueprint $table) {
                $table->id()->comment('自動採番ID');
                $table->unsignedBigInteger('sys_player_id')->comment('sys_playerテーブルのID');
    ...
    {
        foreach ($this->connections as $connection) {
            Schema::connection($connection)->dropIfExists('trx_reward_track_milestone');
            Schema::connection($connection)->dropIfExists('trx_reward_track_line');
            Schema::connection($connection)->dropIfExists('trx_reward_track');
        }
    }
};
