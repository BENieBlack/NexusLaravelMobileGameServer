...
class SysPlayerDeviceRepositoryTest extends TestCase
{
    ...
    public function test_create_device_creates_device_and_returns_with_id(): void
    {
        ...
        $this->assertEquals('test-device-uuid-003', $sysPlayerDevice->uuid);
        $this->assertEquals($deviceInfo, $sysPlayerDevice->device_info);
        $this->assertNotNull($sysPlayerDevice->last_login_at);

        // Verify it was actually inserted into the database
        $dbDevice = SysPlayerDevice::find($sysPlayerDevice->id);
        $this->assertNotNull($dbDevice);
        $this->assertEquals($sysPlayerDevice->uuid, $dbDevice->uuid);
    }
