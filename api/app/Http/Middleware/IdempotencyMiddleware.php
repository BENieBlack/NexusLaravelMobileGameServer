 ...
 * 
 * キャッシュストア: Redis
 * キャッシュキー: "idempotency:{sys_player_id}:{unique_request_id}:{path}"
 * キャッシュ期間: config/idempotency.php で設定可能（デフォルト: 24時間）
...
class IdempotencyMiddleware
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // プレイヤーIDを取得（認証済みの場合）
        $sysPlayerId = $request->input('authenticated_player_id');
        ...
        // キャッシュキーを生成
        $cacheKey = $this->buildCacheKey($sysPlayerId, $uniqueRequestId, $request->path());
     ...
     * @return string
     */
    private function buildCacheKey(int $sysPlayerId, string $uniqueRequestId, string $path): string
    {
    ...
    private function cacheResponse(string $cacheKey, Response $response): void
    {
        ...
        // gzip圧縮（圧縮レベル: COMPRESSION_LEVEL）
        $compressed = gzencode($jsonData, self::COMPRESSION_LEVEL);

        // config/idempotency.php で設定されたTTLを使用（デフォルト: 86400秒 = 24時間）
