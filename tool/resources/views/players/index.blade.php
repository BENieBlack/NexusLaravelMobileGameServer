...
<style>
    ...
    .tab { padding: 10px 16px; border-radius: 6px; background: #e5e7eb; color: #374151; text-decoration: none; }
    .tab.active { background: #667eea; color: #fff; }
    dl { display: grid; grid-template-columns: 180px 1fr; gap: 10px; }
    dt { color: #6b7280; } dd { word-break: break-all; }
...
<div class="player-page">
    <h1>プレイヤー情報</h1>
    @if ($player)
        <nav class="tabs"><a class="tab {{ $activeTab === 'info' ? 'active' : '' }}" href="{{ route('player.index', ['player_id' => $player->id, 'tab' => 'info']) }}">情報</a><a class="tab {{ $activeTab === 'history' ? 'active' : '' }}" href="{{ route('player.log', ['player_id' => $player->id]) }}">履歴</a></nav>

        @if ($activeTab === 'info')
            <section class="card"><h2>基本情報</h2><dl>
                @foreach ((array) $player as $key => $value)<dt>{{ $key }}</dt><dd>{{ is_scalar($value) || $value === null ? ($value ?? '-') : json_encode($value, JSON_UNESCAPED_UNICODE) }}</dd>@endforeach
            </dl></section>
        @else
            @forelse ($records as $connection => $rows)
                <section class="card"><h2>{{ $connection }}</h2>
                    @if ($rows->isEmpty())<p class="empty">該当データはありません。</p>@else
            ...
            @empty
                <section class="card"><p class="empty">対象テーブルまたはデータがありません。</p></section>
            @endforelse
        @endif
    @else
