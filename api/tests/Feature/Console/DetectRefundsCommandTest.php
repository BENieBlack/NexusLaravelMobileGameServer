...
class DetectRefundsCommandTest extends TestCase
{
    ...
    {
        $this->insertPurchaseLog('req-1', 'GPA.1111', 'google_play');
        $this->insertPurchaseLog('req-2', 'GPA.2222', 'google_play');
    ...
    {
        $this->insertPurchaseLog('req-1', 'GPA.1111', 'google_play');
        $this->insertPurchaseLog('req-1:refunded', 'GPA.1111', 'google_play', status: 'refunded');
    ...
    {
        $this->insertPurchaseLog('req-1', '1000000000000001', 'app_store');
        $this->insertPurchaseLog('req-2', 'GPA.2222', 'google_play');

        $facade = Mockery::mock(BillingFacade::class);
        $facade->shouldReceive('isRefunded')
            ->with('app_store', '1000000000000001')
            ->andThrow(new PlatformApiException('App Store Server API is not configured'));
        $facade->shouldReceive('isRefunded')
            ->with('google_play', 'GPA.2222')
            ->andReturnTrue();
        $this->app->instance(BillingFacade::class, $facade);
    ...
    {
        $this->insertPurchaseLog('req-old', 'GPA.OLD', 'google_play', systemAt: now()->subDays(60)->format('Y-m-d H:i:s'));
