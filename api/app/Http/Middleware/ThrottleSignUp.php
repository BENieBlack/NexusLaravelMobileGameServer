<?php

namespace App\Http\Middleware;

use Closure;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Cache;
use Symfony\Component\HttpFoundation\Response;

/**
 * ThrottleSignUp Middleware
 * 
 * sign_upエンドポイントへのレート制限を行います。
 * 
 * 制限の種類:
 * 1. IPアドレスごとの制限: 1時間に10回まで
 * 2. デバイスIDごとの制限: 1時間に3回まで
 * 
 * 連打攻撃やアカウント大量生成を防止します。
 */
class ThrottleSignUp
{
    /**
     * IPアドレスごとの最大試行回数
     */
    private const MAX_ATTEMPTS_PER_IP = 10;

    /**
     * デバイスIDごとの最大試行回数
     */
    private const MAX_ATTEMPTS_PER_DEVICE = 3;

    /**
     * レート制限の期間（秒）
     * デフォルト: 1時間
     */
    private const RATE_LIMIT_WINDOW = 3600;

    /**
     * Handle an incoming request.
     *
     * @param  \Closure(\Illuminate\Http\Request): (\Symfony\Component\HttpFoundation\Response)  $next
     */
    public function handle(Request $request, Closure $next): Response
    {
        $ip = $request->ip();
        $deviceId = $request->input('device_id');

        // 1. IPアドレスベースのレート制限
        $ipKey = "signup_rate_limit:ip:{$ip}";
        $ipAttempts = Cache::get($ipKey, 0);

        if ($ipAttempts >= self::MAX_ATTEMPTS_PER_IP) {
            return response()->json([
                'error' => 'TOO_MANY_REQUESTS',
                'message' => 'Too many sign up attempts from this IP address. Please try again later.',
                'retry_after' => Cache::get("{$ipKey}:ttl", self::RATE_LIMIT_WINDOW),
            ], 429);
        }

        // 2. デバイスIDベースのレート制限
        if ($deviceId) {
            $deviceKey = "signup_rate_limit:device:{$deviceId}";
            $deviceAttempts = Cache::get($deviceKey, 0);

            if ($deviceAttempts >= self::MAX_ATTEMPTS_PER_DEVICE) {
                return response()->json([
                    'error' => 'TOO_MANY_REQUESTS',
                    'message' => 'Too many sign up attempts from this device. Please try again later.',
                    'retry_after' => Cache::get("{$deviceKey}:ttl", self::RATE_LIMIT_WINDOW),
                ], 429);
            }
        }

        // リクエストを処理
        $response = $next($request);

        // 成功/失敗に関わらずカウントを増やす（DoS対策）
        // IPカウント
        $newIpAttempts = $ipAttempts + 1;
        Cache::put($ipKey, $newIpAttempts, self::RATE_LIMIT_WINDOW);
        Cache::put("{$ipKey}:ttl", $this->getRemainingSeconds($ipKey), self::RATE_LIMIT_WINDOW);

        // デバイスIDカウント
        if ($deviceId) {
            $newDeviceAttempts = ($deviceAttempts ?? 0) + 1;
            Cache::put($deviceKey, $newDeviceAttempts, self::RATE_LIMIT_WINDOW);
            Cache::put("{$deviceKey}:ttl", $this->getRemainingSeconds($deviceKey), self::RATE_LIMIT_WINDOW);
        }

        // レート制限情報をヘッダーに追加
        return $response->withHeaders([
            'X-RateLimit-Limit' => self::MAX_ATTEMPTS_PER_IP,
            'X-RateLimit-Remaining' => max(0, self::MAX_ATTEMPTS_PER_IP - $newIpAttempts),
        ]);
    }

    /**
     * キャッシュの残り時間を取得
     * 
     * @param string $key キャッシュキー
     * @return int 残り秒数
     */
    private function getRemainingSeconds(string $key): int
    {
        // キャッシュの有効期限を取得（Laravelのキャッシュ実装に依存）
        // 正確な残り時間が取れない場合はウィンドウ全体の時間を返す
        return self::RATE_LIMIT_WINDOW;
    }
}
